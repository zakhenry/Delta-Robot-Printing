...
void DeltaRobot::calculateWorkingCubicSpaceLimits(){
    ...
    workingCubicSpaceSize = bestSideLength/effectorSideLength; //scale it back down to 0-1 range
...
float DeltaRobot::calculateFitness(float baseSideMultiplier, float upperArmMultiplier, float lowerArmMultiplier, float&elapsedTime){ //main fitness function for GA
    ...
    if (workingPointCloud.size()>0&&cartesianPointCloud.size()>0&&workingPointsInCubicSpace.size()>0){
        
        cout << "cubic working space is sized:  "<<workingCubicSpaceSize<<endl;
        
        if (workingCubicSpaceSize>0.8){
            fitness = workingPointsInCubicSpace.size();
            
            cout << "fitness found to be "<<fitness<<endl;
        }
    }
